...
def run_module():
    ...
    if not exec_path.exists():
        module.fail_json(f'executable {module.params["executable"]} does not exist', **result)
    elif not exec_path.is_file():
        module.fail_json(f'executable {module.params["executable"]} is not a file', **result)
    ...
    if not cos_path.exists():
        module.fail_json(f'/sys/fs/resctrl/{module.params["cos_name"]} does not exist', **result)

    # get current tasks file
    tasks_path = cos_path.joinpath("tasks")
    got = tasks_path.open().read().splitlines()
    if got:
        module.warn(f"COS {module.params['cos_name']} is already in use by PID(s) {got}")

    # set output log file
    if module.params['log_file']:
        result['log_file'] = module.params['log_file']
    else:
        result['log_file'] = LOG_PATH + datetime.datetime.now(). \
                strftime('%Y%m%d%H%M%S') + '-' + str(uuid.uuid4().hex)[:4] + ".log"
    try:
        log_file = Path(result['log_file']).open('w')
    except Exception as e:
        module.fail_json(f"log_file {result['log_file']} can not be opened for writing: {e.args}",
                **result)
    ...
    # start executable
    result['pid'] = subprocess.Popen(command,
