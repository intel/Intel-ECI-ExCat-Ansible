...
The terms class, cos and buffer are used interchangably within this project. `cos0` is the cache space that all processes use per default and that usually occupies the whole cache after startup. The more exclusive buffers we create, the smaller `cos0` gets and thus all remaining processes have less and less cache available. To ensure a minimum size is available for the default cos, one of the variables that can be specified is `min_defaultclass_size` (see 3rd step in Section [Integration into existing project](#integration-into-existing-project)). It defines the minimum amount of cache used for `cos0` in percentage of the whole size for the specified level.
